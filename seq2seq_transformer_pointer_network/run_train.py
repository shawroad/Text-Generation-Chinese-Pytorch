...
def create_masks(question, reply_input, reply_target):
    ...
    # batch_size, max_len = question.size()
    # question_mask = question_mask.unsqueeze(1).expand(batch_size, max_len, max_len).unsqueeze(1)
...
def evaluate(valid_data_loader):
    ...
    for step, batch in enumerate(valid_data_loader):
        with torch.no_grad():
            ...
            # loss = loss_func(output, t_input_ids_output, pad_id=0)
...
def loss_func_ori(logits, labels, pad_id, smoothing=False):
    if smoothing:
...
if __name__ == '__main__':
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in enumerate(train_data_loader):
            ...
            output = model(s_input_ids, s_input_ids_mask, t_input_ids_input, t_input_ids_input_mask)
            # print(s_input_ids.size())   # torch.Size([8, 123])
            # print(t_input_ids.size())  # torch.Size([8, 24])
            # print(t_input_ids_input.size())   # torch.Size([8, 23])
            # print(output.size())   # torch.Size([8, 23, 5121])
            loss = loss_func(output, t_input_ids_output, t_input_ids_output_mask)
