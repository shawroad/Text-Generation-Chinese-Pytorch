...
class Transformer(nn.Module):
    ...
    def decode(self, target_words, target_mask, src_embeddings, src_mask, src_words, is_pointer=True):
        ...
        decoded = self.logit(tgt_embeddings)
        if is_pointer:
            x = self.switch(decoded)
            p_pointer = torch.sigmoid(x) / 10

            if torch.max(src_words) + 1 > decoded.shape[-1]:
                extended = Variable(
                    torch.zeros((decoded.shape[0], decoded.shape[1], torch.max(src_words) + 1 - decoded.shape[-1]))).to(
                    decoded.device)
                decoded = torch.cat((decoded, extended), dim=2)

            output = ((1 - p_pointer) * F.softmax(decoded, dim=2)).scatter_add(
                2, src_words.unsqueeze(1).repeat(1, decoded.shape[1], 1),
                p_pointer * attention[:, 3]) + 1e-10
            out = torch.log(output)
            return out
        else:
            out = F.log_softmax(decoded, dim=2)
            return out
    ...
    def forward(self, src_words, src_mask, target_words, target_mask):
        ...
        # [SOS, xx, xxx, ] -》[2, xx,xx]
        out = self.decode(target_words, target_mask, encoded, src_mask, src_words)
        return out
