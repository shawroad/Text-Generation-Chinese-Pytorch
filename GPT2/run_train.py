...
from data_helper import GPT2Dataset, load_data
...
def get_model(pretrain_model_path=None):
    ...
    if pretrain_model_path is not None:
        ...
        model_config = modeling_gpt2.GPT2Config.from_json_file(args.pretrain_model_config_path)
...
if __name__ == '__main__':
    ...
    tokenizer = BertTokenizer.from_pretrained(args.pretrain_model_path)   # 词表

    # 自定义加了一个起始和终止字符
    num_added_toks = tokenizer.add_special_tokens({'bos_token': '[BOS]', 'eos_token': '[EOS]'})
    print("We have added", num_added_toks, "tokens")
    # 加载GPT2模型
    model = get_model(args.pretrain_model_path)
    model.resize_token_embeddings(len(tokenizer))   # 重置embed matrix
    ...
    train_dataset = GPT2Dataset(train_data_list, args.max_len)
