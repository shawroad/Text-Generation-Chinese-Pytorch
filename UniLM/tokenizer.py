...
def load_bert_vocab(bert_vocab_path) -> Dict[str, int]:
    """
    加载官方中文bert模型字典
    """
...
class BasicTokenizer(object):
    ...
    def ids_to_tokens(self, ids):
        ...
        id序列转换为对应的token序列
        """
        return [self.id_to_token(i) for i in ids.cpu().numpy().tolist()]
...
if __name__ == "__main__":
    # 加载bert当前预训练模型对应的词表
    word2idx = load_bert_vocab('./roberta_pretrain/vocab.txt')
    # print(word2idx)

    tokenizer = Tokenizer(word2idx)
    input_ids = tokenizer.encode('王浩泽荣')
    print(input_ids)
    exit()
